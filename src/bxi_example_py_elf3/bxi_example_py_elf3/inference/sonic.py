...
class SonicTeleopPolicy:
    ...
    def __init__(
        ...
        self.smpl_control_zmq_host = smpl_control_zmq_host or os.environ.get(
            ...
            os.environ.get("SMPL_REF_CONTROL_HOST", self.smpl_ref_zmq_host),
        )
    ...
    def _active_reference(
        self,
    ) -> tuple[Optional[SmplReferenceFrame], str, float]:
        ...
        if live is not None:
            if live.stream_epoch is not None and live.stream_epoch != self.stream_epoch:
                ...
                # A new stream epoch has its own heading; robot and action
                # histories deliberately remain continuous.
            ...
            # Keep the last complete live reference indefinitely.  The policy
            # still evaluates it against current robot proprioception.
            return live, "live", now_mono

        self.live_reference_stale = False
        if self.require_live_reference:
            return None, "waiting", now_mono
    ...
    def inference_step(
        ...
        if source == "live":
            self.last_status = (
                "stale_hold" if self.live_reference_stale else "live_reference"
            )
            # ACK only after the complete model/action path succeeded.  If the
            # control channel is unavailable, inference safely continues on
            # the held reference.
