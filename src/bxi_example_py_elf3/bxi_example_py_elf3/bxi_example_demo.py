"""Official ELF3 ROS2 controller demo.

FILE ROLE / SOURCE OF TRUTH:
For ELF3 sim2sim topic/control semantics, this BXI package is the authority
when it conflicts with GR00T-side standalone MuJoCo smoke/probe scripts:
  /home/huangchenwei/ros2_ws/bxi_rl_controller_ros2_example-main/src/bxi_example_py_elf3/

This node publishes ``communication.msg.ActuatorCmds`` for 29 named ELF3
actuators. Policy integration should preserve this ROS2 command interface and
map actions by joint name, not by raw MuJoCo/IsaacLab index order.
"""
