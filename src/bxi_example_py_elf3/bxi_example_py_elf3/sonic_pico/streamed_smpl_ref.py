"""Policy-owned streamed SMPL reference merger.

This is the Python equivalent of the official SONIC StreamedMotionMerger and
current-frame guard.  It is deliberately independent of ROS, ZMQ and the PICO
Bridge so the control thread is the sole playback-clock owner.
"""
...
def _classify_frame_progress(
    newest_frame: int, previous_newest_frame: int | None
) -> str:
...
class StreamedSmplRefMerger:
    ...
    def build_smpl_ref(
        ...
        *,
        ...
        Call :meth:`advance_after_successful_tick` only after the ONNX/action
        path succeeds.  This preserves the official gather/infer/advance order
        and prevents a failed inference from consuming a reference frame.
        """
        ...
        newest_global_frame = (
            ...
            + (self.timesteps - 1) * self.frame_step
        )
        lead_frames = (
            newest_global_frame - current_global_frame
        ) // self.frame_step
        candidate = current_local + 1
